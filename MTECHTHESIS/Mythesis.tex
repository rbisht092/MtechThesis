...
\makeatletter
% Old list files may contain \addvspace{10\p@}. If @ is tokenized as a
% normal character while the file is read, make \p consume it and expand to pt.
\let\thesis@oldaddvspace\addvspace
\renewcommand*{\addvspace}[1]{%
  {\def\p##1{pt}\thesis@oldaddvspace{#1}}%
}
% The legacy thesis class writes fake "Chapter/Figure/Table" rows as part
% entries in .toc/.lof/.lot. In current LaTeX those rows produce spurious
% numbers and unit errors, so suppress part entries in these front-matter lists.
